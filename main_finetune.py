...
from load_dataset import load_dataframes, build_dataset, build_dataLoader, build_transform, load_dataset
...
def get_args_parser():
    ...
    parser.add_argument('--head_init', type=str2bool, default=True,
                        help='Reinitialize classifier head when fine-tuning. Set false to keep a matching checkpoint head.')
...
def main(args):
    ...
    # tr_df, val_df, test_df, class_names, num_classes = load_datafram(
    #     batch_size=args.batch_size,
    #     num_workers=args.num_workers,  # Set to 0 for Windows, increase for Linux/Mac
    ...
    if args.disable_eval:
        ...
        dataset_val = build_dataset(val_df, val_transformation)
    ...
    data_loader_train = build_dataLoader(dataset_train, batch_size=args.batch_size, num_workers=args.num_workers)
    ...
    model = timm.models._factory.create_model(
        args.model,
        pretrained=True,
    ...
    if args.finetune:
        ...
        # By default, fine-tuning starts with a fresh classifier head.
        # Set --head_init false to keep a matching head from the checkpoint.
        if args.head_init or removed_head:
            trunc_normal_(model.head.fc.weight, std=2e-5)
    ...
    if args.layer_decay < 1.0 or args.layer_decay > 1.0:
        ...
        if args.layer_decay_type == 'group': # applies for Base and Large models
            ...
            depths = [len(stage.blocks) for stage in model.stages]
